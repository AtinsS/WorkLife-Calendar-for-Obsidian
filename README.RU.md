# WorkLife Calendar for Obsidian

> **Всё в одном:** умный календарь, трекер задач и привычек, учёт времени, контроль веса и финансовый планировщик, связанные в единую экосистему внутри Obsidian.

<div align="center">

[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](https://opensource.org/licenses/MIT)
[![Obsidian](https://img.shields.io/badge/Obsidian-7C3AED?logo=obsidian&logoColor=white)](https://obsidian.md)
[![UI: Svelte](https://img.shields.io/badge/UI-Svelte-FF3E00?logo=svelte&logoColor=white)](https://svelte.dev/)
[![TypeScript](https://img.shields.io/badge/TypeScript-5.x-3178C6?logo=typescript&logoColor=white)](https://www.typescriptlang.org/)


</div>

---

![alt text](animate.gif)

[**English README**](https://github.com/AtinsS/WorkLife-Calendar-for-Obsidian/blob/master/README.md)

## 💡 Почему этот плагин существует

У многих рабочих процессов одна и та же проблема: задачи живут в одном месте, календарь — в другом, учёт времени — в третьем, а финансы и отчёты собираются вручную в таблицах. В итоге одни и те же данные приходится вводить несколько раз.

**Этот плагин решает именно эту проблему.** Он не просто добавляет ещё один календарь или трекер. Он связывает планирование, выполнение и анализ в одной системе:
- Одна задача влияет на календарь.
- Календарь влияет на учёт времени.
- Время влияет на расчёт дохода.
- Доход формирует автоматическую аналитику.
- **Мобильный опыт:** работа с задачами из телефона в Obsidian без лишней боли.

---

## 🚀 Для кого это

- **Фрилансеры и разработчики**, которым нужно считать стоимость работы по ставке.
- **Дизайнеры и консультанты**, ведущие несколько проектов одновременно.
- **Студенты и исследователи**, которым важна связка дедлайнов, привычек и продуктивности.
- **Любители автоматизации**, которые хотят, чтобы система работала за них (уведомления, отчёты).

---

## ⚙️ Основной Workflow

```text
Проект 
  ↓
Задача (с оценкой времени и ставкой)
  ↓
Календарь / Расписание (планирование слотов)
  ↓
Учёт времени (факт vs план)
  ↓
Доход и расходы (авторасчёт)
  ↓
Аналитика (графики и отчёты)
```

---

## 📦 Установка

### Через BRAT (Рекомендуется)
1. Установите плагин [BRAT](https://github.com/TfTHacker/obsidian42-brat).
2. Откройте настройки BRAT → **Add Beta Plugin**.
3. Вставьте ссылку: `https://github.com/AtinsS/obsidian-calendar-plugin-remastered`
4. Нажмите **Add Plugin**.

### Вручную
1. Скачайте архив или клонируйте репозиторий.
2. Скопируйте `main.js`, `manifest.json` и `styles.css`.
3. Поместите их в папку `.obsidian/plugins/calendar-plugin-remastered/` (создайте её, если нет).
4. Включите плагин в *Настройки → Сторонние плагины*.

---

## ☕ Поддержка / Support

Если плагин экономит ваше время и помогает в работе, вы можете поддержать разработку:
- ⭐ Поставьте звезду репозиторию.
- [☕ Угостить автора кофе с булочкой](https://boosty.to/atins/donate).

---

<details>
<summary><h3>✨ Подробные возможности (раскрыть)</h3></summary>

# 📅 Календарь и расписание
- День / неделя / месяц на **FullCalendar** с drag & drop.
- Индикаторы задач и привычек прямо в сетке.
- Создание задач кликом, изменение времени — перетаскиванием.
- **Погода** на каждый день недели (Open-Meteo, без ключей).
- Адаптивное мобильное расписание.

# ✅ Задачи и тайм-менеджмент
- **4 статуса:** Сделать → В работе → На паузе → Готово.
- **Быстрое добавление** — `Ctrl+Alt+N` из любого места. Парсинг естественного языка с подсветкой:
  - Проект: `@Работа 14-15 совещание`
  - Время: `14:00 купить молоко`, `14-15 совещание`, `с 16:00 по 18:00 встреча`
  - Дата: `завтра купить молоко`, `пятница отчёт`, `25.07 встреча`, `+3 задача`
  - Приоритет: `! срочно`, `~ средний`, `- низкий`
  - Дата и время в любой позиции: `завтра в 14:00 купить молоко`
  - `Enter` → расширенный редактор с предзаполненными данными.
- **Канбан-доска** — 4 колонки, drag & drop между ними, создание задач в колонке «Сделать».
  - Карточки: цвет проекта, время, дедлайн, приоритет, бейдж рабочей задачи, повторение, ссылка на заметку, живой таймер.
  - Фильтры: Сегодня / Все / Конкретная дата / Проект.
  - Контекстное меню по правому клику.
- **Повторяющиеся задачи:** ежедневно / еженедельно / ежемесячно с интервалом.
- **Проекты:** группировка с цветовой маркировкой.
- **Таймер:** учёт времени с автовозобновлением при перезапуске Obsidian, живой таймер на карточках.
- **Чек-листы** для каждой задачи.
- **Дедлайны и оценки:** план vs факт, уведомления о сроках.
- **Синхронизация** с плагинами Tasks и Dataview через `.md`-файлы.

# 🔄 Трекер привычек
- Гибкая периодичность (дни недели, день месяца).
- Количественные цели.
- Стрики и индикаторы на календаре.
- **Режимы отображения:** в панели задач (по умолчанию) / отдельной вкладкой / скрыть. Настройка: Общее → «Режим привычек».
- **Полный CRUD** из панели привычек и дашборда.
- **История выполнения** — график в аналитике.

# ⚖️ Контроль веса
- **Ежедневный ввод** — дата, вес (кг), заметка. Одно измерение в день.
- **Скользящее среднее** — MA за 7 и 30 измерений (без шума от скачков дня).
- **Цель и прогресс** — старт / цель, прогресс-бар, сколько осталось (снижение или набор).
- **График** — в том же стиле, что и «Время и проекты» (область, подсказки при наведении).
- **Быстрый ввод в приветствии** — «Вес сегодня» одной строкой.
- **История** — сворачиваемый список измерений с удалением.
- Переключатель: *Общее → Контроль веса* (также скрывает поле в приветствии).

# 📊 Вкладки аналитики
Аналитика разделена на вкладки: **Вес · Привычки · Время · Заработок**.
- Привычки: история выполнения и карточки привычек.
- Время и проекты: итоги, график, анимированная круговая диаграмма распределения, таблица проектов.
- Заработок: факт/план за месяц, итог за год, столбики по месяцам.

# 💰 Финансы и аналитика
- **Доходы:** автоматически по ставке из рабочих задач или вручную.
- **Бюджет:** категории расходов с иконками и правилами распределения.
- **Накопления:** цели с процентом выполнения.
- **Аналитика:** диаграммы по проектам, динамика доходов/расходов по месяцам, план vs факт.

# 🎨 Внешний вид и UI
- Настраиваемый акцентный цвет.
- Стеклянные панели (glassmorphism) с фоном и прозрачностью.
- **Панель информации** под вкладками (дата, время, погода, задачи).
- **Дашборд** для заметок, задач и привычек (создание, редактирование, удаление).

# 🌍 Локализация
- **Русский и English**, переключение в настройках.
- **Системный язык** — автоопределение.
- **Начало недели:** Понедельник / Воскресенье / по языку. Влияет на календарь, расписание, повторяющиеся задачи и привычки.
- Переведено всё: интерфейс, настройки, уведомления, погода, аналитика.

# ⛅ Погода
- **Вкладка Погода** из сайдбара при выборе дня.
- **Погода в недельном виде календаря** — проще планировать неделю.
- **Выбор провайдера:** Open-Meteo, OpenWeatherMap, WeatherApi, Visual Crossing.

# 🤖 Извлечение задач с помощью AI (Ollama)
- **Локальный AI** — задачи из `.md`-заметок через модель Ollama. Данные не покидают компьютер.
- **Умный парсинг** — понимает недели (`## Неделя 1`), дни (`### День 3`), заголовки, чекбоксы, оценки времени.
- **Привязка к проекту** для извлечённых задач.
- **Планирование времени** — AI предлагает начало; ручное редактирование окончания; авто-планирование.
- **Подзадачи** — чекбоксы становятся пунктами чеклиста.
- **Правый клик** по `.md`-файлу → «Извлечь задачи с помощью AI».
- **Настройки** — URL Ollama, модель, лимит контекста, тест соединения, живой тест AI.
- **Только десктоп** — AI-извлечение недоступно на мобильных.
</details>

<details>
<summary><h3>🔗 Синхронизация и интеграции (раскрыть)</h3></summary>

### Новый формат хранения данных
Плагин может хранить данные в JSON-формате в папке `calendar-data/` в корне хранилища. При включении функции "Синхронизировать в корень хранилища" он становится основным форматом хранения данных, обеспечивающим быструю загрузку и синхронизацию данных через:
- **WebDAV** (Яндекс.Диск, OneDrive и др.)
- **Obsidian Sync** / **Remotely Save**
- **iCloud** / **Google Drive**
- **Syncthing**


> [!WARNING] Финансовые данные
> Если вы ведёте учёт финансов в плагине и используете облачную синхронизацию, данные о доходах и расходах будут храниться в открытом виде в облаке. Рекомендуется использовать абстрактные названия проектов или настроить исключение папки `calendar-data/` из синхронизации.

### Внешние календари (Нужна сихронизаиция чрез Git)
1. Создайте [GitHub Personal Access Token](https://github.com/settings/tokens) (classic) с правами (scope) `gist`.
2. Вставьте токен в настройки плагина и нажмите **«Синхронизировать»**.
3. Плагин создаст Gist с `.ics` файлом и выдаст ссылку.
4. Добавьте эту ссылку в свой календарь через функцию «Подписка по URL».

### Интеграция с форматом Tasks (опционально)
При включённой настройке «Tasks plugin sync» плагин создаёт `.md` файлы для задач, чтобы они были видны в плагинах Tasks и Dataview. Это **дополнительная** фича — основное хранение остаётся в JSON. Пример генерируемого файла:
```markdown
---
task_id: abc123
title: Купить молоко
status: todo
date: day-2024-10-25
priority: medium
---
- [ ] Купить молоко 📅 2024-10-25 🛫 14:30 🔼
```
*(Поддерживаемые статусы: `- [ ]` todo, `- [/]` progress, `- [-]` paused, `- [x]` done)*

> [!NOTE]
> Опциональная синхронизация с `.md` файлами (через настройку «Tasks plugin sync») предназначена для совместимости с плагинами [Tasks](https://github.com/obsidian-tasks-group/obsidian-tasks) и [Dataview](https://github.com/blacksmithgu/obsidian-dataview).

</details>

<details>
<summary><h3>🔔 Уведомления (раскрыть)</h3></summary>

В плагине встроена система уведомлений, чтобы вы не пропустили важное.

| Тип | Когда срабатывает |
| :--- | :--- |
| **Локальные (браузер)** | За N минут до начала, при просрочке, при превышении лимита времени, в день дедлайна. |
| **На смартфон (ntfy.sh)** | Дублирование уведомлений на телефон. Работает, даже когда Obsidian закрыт. |

### Настройка ntfy.sh

Простой способ получать уведомления на телефон:
1. Установите приложение [ntfy.sh](https://ntfy.sh/) на телефон.
2. В настройках плагина включите **ntfy.sh** и задайте топик.
3. Подпишитесь на этот топик в приложении.

> [!CAUTION] Безопасность
> Используйте уникальный топик (например, сгенерированный UUID вроде `a7f9b2c4-8e1d-4f3a-9c5b-2d6e8f0a1b3c`), чтобы никто другой не мог подписаться на ваши уведомления. Плагин отправляет только триггеры («Просрочено: Название задачи»), а не финансовые данные или полные тексты.

</details>

<details>
<summary><h3>🧭 UI Виджеты в заметках (раскрыть)</h3></summary>

Вставьте блок кода в любую заметку для создания панели быстрой навигации по разделам плагина:

````markdown
```calendar-nav
schedule:Расписание
tasks:Задачи
finance:Финансы
analytics:Аналитика
```
````
Доступные ключи: `schedule`, `tasks`, `finance`, `analytics`.

**Кастомизация стиля** (первая строка начинается с `%`):
````markdown
```calendar-nav
%color:#fff;bg:#333;radius:20px;size:14px;accent:#5f99e1
schedule:Расписание
tasks:Задачи
```
````
Параметры стиля: `color` (текст), `bg` (фон), `radius` (скругление), `size` (размер шрифта), `accent` (цвет при наведении).

### Дашборд и приветствие

Нажмите на страницу правой кнопкой мыши "Добавить Дашборд" или "Вставить приветствие" для создания нового дашборда или приветствия на странице.
![alt text](image-2.png)

В приветствии можно показать быстрый ввод веса и поиск заметок — оба пункта переключаются в *Дашборд → Кнопки приветствия*.
Виджеты дашборда (задачи / привычки / цели) настраиваются в *Дашборд → Виджеты дашборда*.

</details>

---

## 🐛 Issues и баг-репорты

Нашли баг или есть предложение по функционалу? Откройте issue на GitHub:

**[Открыть Issue](https://github.com/AtinsS/WorkLife-Calendar-for-Obsidian/issues)**

При сообщении о баге, пожалуйста, укажите:
- Версия Obsidian
- Версия плагина
- Шаги для воспроизведения
- Ожидаемое и фактическое поведение
- Ошибки в консоли (если есть): *Ctrl+Shift+I → вкладка Console*

---

<div align="center">
  <sub>Разработано с вниманием к деталям для сообщества Obsidian</sub><br>
  <sub>Автор: <a href="https://github.com/AtinsS">@AtinsS</a></sub><br>
  <sub>Лицензия: <a href="https://opensource.org/licenses/MIT">MIT</a></sub>
</div>


