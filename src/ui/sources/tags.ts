...
function getNoteTags(note: TFile | null, appInstance: App): string[] {
  if (!note) {
    return [];
  }

  const { metadataCache } = appInstance;
  const frontmatter = metadataCache.getFileCache(note)?.frontmatter;

  const tags: string[] = [];

  if (frontmatter) {
    const frontmatterTags: string[] = parseFrontMatterTags(frontmatter) ?? [];
...
function getFormattedTagAttributes(note: TFile | null): Record<string, string> {
  ...
  const appInstance: App | null = get(app);
  if (!appInstance) return attrs;
  const tags: string[] = getNoteTags(note, appInstance);
